          ...
          console.log("Form data received:", formDataArray);
              if (formDataArray && formDataArray.length > 0) {
                          ...
                          formDataArray.forEach(function (formData, index) {
                              ...
                              detailsCell.innerHTML = '<a href="#" onclick="handleDetailsClick(\'' + formData.formName + '\', \'' + formData.formId + '\', \'' + formData.studentName + '\', \'' + user_id + '\', \'' + formData.status + '\', \'' + (index+1) + '\')"><ion-icon name="folder-open-outline"></ion-icon></a>';
...
// Function to handle click on details link
function handleDetailsClick(formName, formId, studentName, studentId, formStatus, index) {
    if (formName === 'เพิ่มรายวิชา' || formName === 'ถอนรายวิชา') {
        window.location.href = `AddDropForm/add_drop.html?studentId=${studentId}&formId=${formId}&studentName=${studentName}&status=${formStatus}&order=${index}`;
    } else if (formName === 'ขอลาออก') {
        window.location.href = `QuitForm/QuitForm.html?studentId=${studentId}&formId=${formId}&studentName=${studentName}&status=${formStatus}&order=${index}`;
    } else if (formName === 'ขอผ่อนผัน') {
        window.location.href = `LateForm/lateform.html?studentId=${studentId}&formId=${formId}&studentName=${studentName}&status=${formStatus}&order=${index}`;
    } else if (formName === 'อื่นๆ') {
        window.location.href = `OtherForm/otherform.html?studentId=${studentId}&formId=${formId}&studentName=${studentName}&status=${formStatus}&order=${index}`;
    }
