 ...
 // Add event listener for screen resize
  ...
  checkScreenSize();
    ...
    // Fetch student data by studentId
        $.ajax({
          type: "GET",
          url: `/api/form/getOtherFormByFormId?formId=${form_id}`,
          contentType: "application/json",
          success: function (otherForm) {
              if (otherForm) {
              disableAllInputs()

                  // Populate the input fields with fetched data
                  document.getElementById("info").value = otherForm.info;
              } else {
          ...
          // Fetch student data by studentId
              $.ajax({
                type: "GET",
                url: `/api/form/getTrackingOtherForm?formId=${form_id}`,
                contentType: "application/json",
                success: function (TrackingOther) {
                console.log("Form data received:", TrackingOther);
                ...
                dateTimeCell.innerHTML = TrackingOther.dateTime; // Replace with the actual property name
                if (TrackingOther.status === 'อนุมัติ') {
                    statusCell.innerHTML = '<div style="display: inline-block; border-radius: 17px; border: 1px solid #0C7348; background: rgba(53, 216, 148, 0.70); padding: 15px 40px;">' + TrackingOther.status + '</div>';
                } else if (TrackingOther.status === 'ปฏิเสธ') {
                    statusCell.innerHTML = '<div style="display: inline-block; border-radius: 17px; border: 1px solid #A41B3E; background: rgba(228, 128, 153, 0.70); padding: 15px 40px;">' + TrackingOther.status + '</div>';
                } else {
                    statusCell.innerHTML = '<div style="display: inline-block; border-radius: 17px; border: 1px solid #E9BE2B; background: rgba(253, 225, 129, 0.70); padding: 15px 40px;">' + TrackingOther.status + '</div>';
                }

                formNameCell.innerHTML = TrackingOther.formName;
...
function previous() {
  ...
  function submitForm(event) {
     ...
     if(form_status === 'รออาจารย์ที่ปรึกษาอนุมัติ'){
      ...
      const formData = $("#otherForm").serializeArray();
      ...
      $.ajax({
        type: "POST",
        url: `/api/form/updateOtherForm?formId=${formId}`, // Replace with your actual endpoint
